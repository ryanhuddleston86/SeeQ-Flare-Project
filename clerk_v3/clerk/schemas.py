...
class AnalyzerUnit:
    ...
    # v3 Gap #1: the OBLIGATION (pollutant / DAR) this analyzer serves —
    # e.g. "NOx", "SO2", "CO", "O2". A unit can carry several obligations at
    # once, each with its own availability. The source rollup intersects
    # WITHIN an obligation, never across a whole unit, so a valid monitor for
    # one pollutant can never mask another pollutant's outage (Doctrine D8).
    # Blank means "unspecified" — all blank-obligation monitors on a unit are
    # grouped together, which reproduces the pre-v3 per-unit intersection.
    Obligation: str = ""
    ...
    with open(path, newline="") as f:
        for r in csv.DictReader(f):
            ...
            rows.append(AnalyzerUnit(
                ...
                # v3 Gap #1: obligation/pollutant — blank/absent column is ""
                Obligation=(r.get("Obligation") or "").strip(),
                DiluentsRole=role,
